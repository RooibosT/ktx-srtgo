...
KTX용 Chromium은 `install.sh`가 Playwright `1.42.0`의 Chromium(확인된 동작 버전)을 프로젝트의 `.cache/ms-playwright`에 함께 설치합니다. `PLAYWRIGHT_BROWSERS_PATH`, 프로젝트 캐시, Linux의 `~/.cache/ms-playwright`, macOS의 `~/Library/Caches/ms-playwright`를 순서대로 탐색하며, 필요하면 `python -m ktxgo --extension-chromium /path/to/chrome`으로 직접 지정할 수 있습니다.
