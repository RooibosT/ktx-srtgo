...
```bash
python3 -m ktxgo \
  --extension-chromium "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
...
```text
# install.sh 기본값 (Linux/macOS)
<프로젝트>/.cache/ms-playwright/chromium-1105/...

# Linux
~/.cache/ms-playwright/chromium-1105/chrome-linux/chrome

# macOS
~/Library/Caches/ms-playwright/chromium-1105/chrome-mac*/Chromium.app/Contents/MacOS/Chromium
```

`ktxgo`는 `PLAYWRIGHT_BROWSERS_PATH`, 프로젝트 캐시, 위 사용자 캐시를 먼저 찾고, 없으면 다른 Playwright Chromium 캐시와 Linux의 `chromium`/`chromium-browser`, macOS의 `/Applications/Chromium.app` 및 `/Applications/Google Chrome.app`을 순서대로 찾습니다. 문제가 있으면 `--extension-chromium`으로 직접 지정하세요.
