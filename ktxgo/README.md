...
keyring set KTX pass      # 비밀번호 (세션 만료 시 TTY 안내에 표시)
```
...
시작
 ├─ 저장된 쿠키로 로그인 확인
 │   ├─ 성공 → headless 모드로 진행
 │   └─ 실패 → 브라우저 창 전환 + 저장된 계정(KTX id/pass)을 TTY에 표시
 │            └─ 사용자 수동 로그인 성공 → 세션 저장 → (필요 시) headless 전환
 │
...
| 카드/계정 | OS keyring | `keyring` 라이브러리 사용 (`KTX id/pass` 수동 로그인 안내 포함) |
...
- **loginProcess**: 최엄격 보호 → 저장 계정을 TTY에 보여주고 visible 브라우저에서 수동 로그인
